...
export function SessionOverview({
	...
	const mostCommonWinner = useMemo(() => {
		const winsByTeams = session.rounds.reduce(
		...
		return Object.entries(winsByTeams).reduce(
			(currentMostCommonWinner, entry) => {
				return entry[1] > winsByTeams[currentMostCommonWinner]
					? entry[0]
					: currentMostCommonWinner;
			},
				...
				<div className="flex items-center gap-3">
					...
					<div>
						<h4 className="text-lg font-medium text-zinc-100">
							{dayjs(session.startedAt).format("DD/MM/YYYY")} at{" "}
							{dayjs(session.startedAt).format("hh:mm")}
						</h4>
						<p className="text-sm text-zinc-400">
							{session.rounds.length} rounds • [duration] total
						</p>
					</div>
				</div>
				<div className="flex items-center gap-3">
					<span className={`role-badge ${mostCommonWinner?.toLowerCase()}`}>
						{cFirst(mostCommonWinner)}
					</span>
				...
				<div className="border-t border-zinc-700 p-4">
					<Table>
						...
						<TableBody>
							{session.rounds
								.sort((roundA, roundB) =>
									dayjs(roundA.startedAt).diff(roundB.startedAt),
								)
								.map((round, index) => (
									<TableRow
										key={round.id}
										className="border-zinc-700 hover:bg-zinc-750"
									>
										<TableCell className="font-medium">Round {index}</TableCell>
										<TableCell>
											{dayjs
												.duration(
													dayjs(round.endedAt).diff(
														dayjs(round.startedAt),
														"seconds",
													),
													"seconds",
												)
										...
										<TableCell>
											<Badge
												className={
													round.winningTeam === "innocents"
														? "bg-green-700"
														: round.winningTeam === "traitors"
															? "bg-red-700"
															: "bg-purple-700"
												}
											>
