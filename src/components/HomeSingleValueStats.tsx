...
export function HomeSingleValueStats({ dateRange }: HomeSingleValueStatsProps) {
	...
	const updateSingleValueStats = useCallback(() => {
		if (dateRange?.from && dateRange?.to) {
			getMapCount(dateRange.from, dateRange.to).then((result) =>
				setMapCount(result ?? 0),
			);

			getRoundCount(dateRange.from, dateRange.to).then((result) =>
				setRoundCount(result ?? 0),
			);

			getRoundWinsByTeam("innocents", dateRange.from, dateRange.to).then(
				(result) => setInnocentWins(result ?? 0),
			);

			getRoundWinsByTeam("traitors", dateRange.from, dateRange.to).then(
				(result) => setTraitorWins(result ?? 0),
			);
		} else {
			setMapCount(0);
			setRoundCount(0);
